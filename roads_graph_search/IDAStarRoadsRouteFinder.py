...
class IDAStarRoadsRouteFinder(RoadsRouteFinder):
    ...
    def _inner_find(self, search_problem: RoadsSearchProblem) -> Optional[List[Node]]:
        path_cost_heuristic_func = partial(
            combined_curr_cost_heuristic,
            roads_map=self.roads_map,
            target_junction_index=search_problem.goal_junction.index,
        )

        route_node = idastar_graph_search(
            search_problem, f=path_cost_heuristic_func, is_tree_search=self.is_tree_search
        )
        if route_node:
