...
class RandomSearchProblemGenerator(NamedTuple):
    ...
    def generate(
        self, min_index: int = MIN_STATE_INDEX, max_index: int = MAX_STATE_INDEX
    ) -> Set[SearchQuery]:
        """
        ...
        while (
            len(generated_problems) < self.problem_count
        ):  # we assume there are enough valid problems in the map
    ...
    def check_search_query_connected(self, source: int, target: int):
        ...
        if (
            search_query_sol_path
            and len(search_query_sol_path) >= self.MIN_SEARCH_PROBLEM_SOLUTION_PATH_LENGTH
        ):
