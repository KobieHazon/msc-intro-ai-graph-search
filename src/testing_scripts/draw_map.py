...
def find_route_timeout(
    route_finder: RoadsRouteFinder, source_index: int, target_index: int, timeout: int = 50
) -> Optional[list]:
    """
...
def get_random_problem_paths(
    ...
    """
    all_problems = [
        (problem_index, *problem_indices)
        for problem_index, problem_indices in enumerate(
            read_search_problems_file(problems_input_file)
        )
    ]  # does row number begin at 0 or 1
...
def export_paths_images(
    problem_index_to_path: Dict[int, List[int]],
    roads_map: Roads,
