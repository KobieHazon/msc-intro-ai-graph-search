...
def map_statistics(roads):
    ...
    sorted_junction_links_counter = [links_num for junction_id, links_num in junction_links_counter.most_common()]
    sorted_junction_link_distance_counter = [link_distance for link_id, link_distance in
                                             junction_link_distance_counter.most_common()]

    return {
        'Number of junctions': len(roads),
        'Number of links': sum(junction_links_counter.values()),
        'Outgoing branching factor': Stat(max=sorted_junction_links_counter[0], min=sorted_junction_links_counter[-1],
                                          avg=statistics.mean(sorted_junction_links_counter)),
        'Link distance': Stat(max=sorted_junction_link_distance_counter[0],
