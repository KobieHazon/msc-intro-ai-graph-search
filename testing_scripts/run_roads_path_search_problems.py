...
from roads_graph_search.utils import huristic_function, compute_path_time
from testing_scripts.RandomSearchProblemGenerator import SearchQuery
from helper_types.Node import path_str


def read_search_problems_file(problems_file_path: str = './problems.csv', problem_count: int = math.inf) \
...
def run_path_search_problems_file(route_finder: RoadsRouteFinder,
    ...
    with open(os.path.join(DEFAULT_RESULTS_FOLDER, export_file_path), 'w') as export_file:
        for source_index, target_index in read_search_problems_file(problems_file_path):
            ...
            path_heuristic_time = huristic_function(roads_map[source_index].lat, roads_map[source_index].lon,
                                                    roads_map[target_index].lat, roads_map[target_index].lon)
            ...
            export_row = export_row_format.format(path=path_str(search_path),
